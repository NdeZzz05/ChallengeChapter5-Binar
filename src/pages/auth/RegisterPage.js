...
export const RegisterPage = () => {
  ...
  const formik = useFormik({
    ...
    validationSchema: yup.object().shape({
      ...
      password: yup
        ...
        .matches(
          /^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[!@#\$%\^&\*])(?=.{8,})/,
          "Kata sandi harus terdapat 8 karakter terdiri dari huruf besar, huruf kecil, angka, dan karakter spesial"
        ),
    }),
  });
  ...
  useEffect(() => {
    if (data?.data?.data?.token) {
      ...
      data?.response?.data?.message === "User has already registered"
    ) {
            ...
            <Container py="4">
              ...
              <Box padding="4" border="" borderRadius="4px" mt="4" px="8">
                ...
                <div className="text-white font-thin flex justify-center items-center pt-[1rem]">
                  Sudah punya akun?
                  <span
                    className=" cursor-pointer font-medium pl-[0.3rem]"
                    onClick={() => navigate(`/`)}
                  >
                    Login sekarang
                  </span>
